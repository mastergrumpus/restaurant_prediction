...
3.  neural_network.ipynb - This notebook is experimental and is unfinished.  A simple network did not perform better than my other models but has the potential to do so if further experimentation is done.
4.  final_models.ipynb - This is where the models are finalized and the ensemble methods are performed to bring the best functioning models together.  This notebook also includes visualizations, code, and markdown to evaluate each model's performance and how each step affected the project's performance.
